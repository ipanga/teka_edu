...
| Supabase migrations and database tests       | `docker login ghcr.io` with the job's own `GITHUB_TOKEN` (anonymous pulls from shared runners are rate-limited) → `supabase db start` (applies all migrations and the dev seed) → `supabase db reset` → `supabase test db` (pgTAP: RLS and access registry, reference data = `content/`, integrity rules) |
