...
| Supabase migrations and database tests       | images pulled from Supabase's `public.ecr.aws` mirror (`SUPABASE_INTERNAL_IMAGE_REGISTRY`; ghcr.io rate-limited the pull on 2026-09-23) → `supabase db start` (applies all migrations and the dev seed) → `supabase db reset` → `supabase test db` (pgTAP: RLS and access registry, reference data = `content/`, integrity rules) |
