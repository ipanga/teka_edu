...
describe("what a batch of content has actually been through", () => {
  ...
  it("matches the canonical September state of both levels", () => {
    ...
    // 3ème Week 3 has had its first full review, which accepted it with modifications.
    expect(stateOf("maternelle-3", 3, ["m3-math-10"])).toBe("reviewed");
    // Weeks 4-5 carry only inherited corrections, so they have never been reviewed.
...
describe("an approval can only come from a full review that accepted the week", () => {
  ...
  it("does not let an inherited correction stand in for a review", () => {
    ...
    // approve anything. Week 3 has since had a real first reading, so it is excluded.
